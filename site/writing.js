...
(function () {
  ...
  function cover(p, sizes) {
    ...
    if (p.cover) {
      ...
      // coverSet: responsive srcset generated by the Notion sync — the browser
      // picks 480/960/1600w by context instead of always fetching full size.
      var resp = p.coverSet
        ? ' srcset="' + esc(p.coverSet) + '" sizes="' + (sizes || '(max-width: 768px) 100vw, 50vw') + '"'
        : '';
      return '<div class="wr-cover' + fit + '"><img src="' + esc(p.cover) + '"' + resp + ' alt="" loading="lazy"></div>';
    }
  ...
  function renderArchive() {
      ...
      featWrap.innerHTML =
        '<div class="wr-feat" data-open="' + esc(feat.id) + '">' +
          '<div class="wr-feat-inner">' +
            '<div class="wr-feat-media">' + cover(feat, '(max-width: 900px) 100vw, 640px') + '<span class="wr-badge-pos">' + badge(feat.tag) + '</span></div>' +
    ...
    grid.innerHTML = list.slice(1).map(function (p) {
      return '<div class="wr-card" data-open="' + esc(p.id) + '">' +
          '<div class="wr-card-media">' + cover(p, '(max-width: 700px) 100vw, 420px') + '<span class="wr-badge-pos">' + badge(p.tag) + '</span></div>' +
  ...
  function renderReader(post) {
    ...
    $('wrArticle').innerHTML =
      ...
      '<div class="wr-article-cover">' + cover(post, '(max-width: 960px) 100vw, 860px') + '</div>' +
    ...
    rail.innerHTML = others.map(function (p) {
      return '<div class="wr-mini" data-open="' + esc(p.id) + '">' +
          '<div class="wr-mini-media">' + cover(p, '320px') + '<span class="wr-badge-pos">' + badge(p.tag, 'mini') + '</span></div>' +
